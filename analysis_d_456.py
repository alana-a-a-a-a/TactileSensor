...
# %%
clawdata_file = 'testing_2026_04_22_sample_d_456.txt'
...
def forEachSample(n):
    ...
    range = np.array([start_t, end_t],dtype=float)
    ...
    Case2 = np.array(Case2, dtype=int)
    Case3 = np.array(Case3, dtype=int)
...
def plot_one_case(Case, c, SampleName, endTOI=None):
    ...
    elif c==2: S = 'POS 30 -> CUR -500'
    elif c==3: S = 'POS 30 -> CUR -1000'
    ...
    #ms[:5]
        ...
        initialp = [t_matrix[t1,1],t_matrix[t1,2],t_matrix[t1,3], t_matrix[t1,4], t_matrix[t1,5],t_matrix[t1,6],t_matrix[t1,7]]
        
        p1 = t_matrix[t1:t2,1] -initialp[0]
        p2 = t_matrix[t1:t2,2] -initialp[1]
        p3 = t_matrix[t1:t2,3] -initialp[2]
        p4 = t_matrix[t1:t2,4] -initialp[3]
        p5 = t_matrix[t1:t2,5] -initialp[4]
        p6 = t_matrix[t1:t2,6] -initialp[5]

        p_zeroed = [p1, p2, p3, p4, p5, p6]

        df = pd.DataFrame({'time': x,'p4': p4,'p5': p5,'p6': p6})      
        df.to_csv(f'SampleD_Case{c}.csv')


        colors = ['blue', 'blueviolet', 'mediumorchid', 'violet', 'magenta', 'deeppink', 'lightpink']
        for p in np.arange(4,7):
            if e == 0: 
                axs[0].scatter(x,p_zeroed[p-1], c=colors[p-1],s=5, label = f"Sensor {p}")
            else:
                axs[0].scatter(x,p_zeroed[p-1], c=colors[p-1],s=5)
...
# %%
plot_one_case(C1, 1, 'Sample D',2)
plot_one_case(C2, 2, 'Sample D',2)
plot_one_case(C3, 3, 'Sample D',2)
