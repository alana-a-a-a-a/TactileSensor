...
class MS5837:
    ...
    def __init__(self, i2c_device, model=None):
        self.i2c_device = i2c_device
        self._model = model if model is not None else MODEL_UNKNOWN
    ...
    def init(self):
        with self.i2c_device:
            self.i2c_device.write(bytes([self._MS5837_RESET]))
        time.sleep(0.01)

        self._C = []
        # Read calibration values and CRC
        for i in range(7):
            reg = self._MS5837_PROM_READ + 2*i
            buf = bytearray(2)
            with self.i2c_device:
                self.i2c_device.write(bytes([reg]))
                self.i2c_device.readinto(buf)
            c = (buf[0] << 8) | buf[1]
            self._C.append(c)
    ...
    def read(self, oversampling=OSR_8192):
        ...
        with self.i2c_device:
            self.i2c_device.write(bytes([self._MS5837_CONVERT_D1_256 + 2*oversampling]))
        time.sleep(2.5e-6 * 2**(8+oversampling))
        buf = bytearray(3)
        with self.i2c_device:
            self.i2c_device.write(bytes([self._MS5837_ADC_READ]))
            self.i2c_device.readinto(buf)
        self._D1 = buf[0] << 16 | buf[1] << 8 | buf[2]

        # Request D2 conversion (temperature)
        with self.i2c_device:
            self.i2c_device.write(bytes([self._MS5837_CONVERT_D2_256 + 2*oversampling]))
        time.sleep(2.5e-6 * 2**(8+oversampling))
        buf = bytearray(3)
        with self.i2c_device:
            self.i2c_device.write(bytes([self._MS5837_ADC_READ]))
            self.i2c_device.readinto(buf)
        self._D2 = buf[0] << 16 | buf[1] << 8 | buf[2]
